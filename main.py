...
class MainTabView(ctk.CTkTabview, LogMixin):
    def __init__(self, master, **kwargs):
        ...
        self.canvas = tk.Canvas(self.tab(self.get()), highlightthickness=0, bg=master.cget('bg'), width=config.CANVAS_WIDTH)
        ...
        self.canvas.create_window(0, 0, window=self.frame, anchor=tk.NW, width=config.CANVAS_WIDTH, height=self.canvas.cget('height'))
        self.update_canvas()
        # self.master.update()
    ...
    def create_temp_frame(self):
        temp_frame = tk.Frame(self.frame, bg='green', height=50)
        for ind in range(3):
